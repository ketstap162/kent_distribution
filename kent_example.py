...
def test_example_normalization(
    showplots=False, verbose=False, gridsize=100, print_grid=True
):
    scale = 1000.0 / gridsize
    print(
        "Calculating the matrix M_ij of values that can be calculated: kappa=%.1f*i+1, beta=%.1f+j*1"
        % (scale, scale)
    )
    with warnings.catch_warnings():
        warnings.simplefilter("error")
        c_grid, dck_grid, dcb_grid = [
            zeros((gridsize, gridsize)) - 1.0 for z in range(3)
        ]
        cnum_grid, dcnum_grid = [
            zeros((gridsize, gridsize), dtype=int32) - 1 for z in range(2)
        ]
        sys.stdout.write(
...
def test_example_mle(showplots=False):
    for k in [
        ...
        if showplots:
            ...
            ax.plot_surface(
                x, y, z, rstride=4, cstride=4, facecolors=colors, linewidth=0
            )
...
def test_example_mle2(num_samples, showplots=False, verbose=False, stepsize=1.0):
    ...
    (
        bias_var_mse_kappa_me,
        bias_var_mse_kappa_mle,
        bias_var_mse_beta_me,
        bias_var_mse_beta_mle,
    ...
    for beta_ratio in beta_ratios:
        ...
        if verbose:
            print(
                "beta (max 2.0) = %s*kappa : kappa (max %.1f) = "
                % (beta_ratio, max_kappa),
            )
        ...
        for kappa in real_kappas:
            ...
            k = kent(
                random.uniform(0, pi), random.uniform(0, 2 * pi), random.uniform(0, 2 * pi), kappa, beta
            )
    ...
    for (name, bias_var_mse_mle, bias_var_mse_me, beta_ratio) in [
        ...
        for mse_me, mse_mle, beta_ratio in zip(mses_me, mses_mle, beta_ratios):
            if mse_me < mse_mle * 0.7:
                print(
                    "MSE of MLE is lower than 0.7 times the moment estimate for %s"
                    % name
                )
            ...
            if beta_ratio >= 0.3:
                if mse_me < mse_mle:
                    print(
                        "MSE of MLE is lower than moment estimate for %s with beta/kappa >= 0.3"
                        % name
                    )
            ...
            if beta_ratio > 0.5:
                if mse_me < 5 * mse_mle:
                    print(
                        "MSE of MLE is not lower than five times the moment estimate %s with beta/kappa >= 0.5"
                        % name
                    )
...
if __name__ == "__main__":
    ...
    if showplots:
        from pylab import export

        export(path="diagram.png", format="png", replace=True)
    exit()
